...
def translate_subtitles_batch(
    subtitles: List[Dict], batch_size: int = 20, target_lang: str = "中文"
) -> List[Dict]:
    """
    批量翻译字幕

    注意：此函数需要在 Skill 环境中调用
    AI 会自动处理翻译逻辑
...
def create_bilingual_subtitles(
    subtitles: List[Dict], output_path: str, english_first: bool = True
) -> str:
    """
    创建双语字幕文件（SRT 格式）
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for i, sub in enumerate(subtitles, 1):
            ...
            start_time = seconds_to_time(
                sub["start"], include_hours=True, use_comma=True
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python translate_subtitles.py <subtitle_file> [output_file] [batch_size]"
        )
        print("\nArguments:")
        ...
        print("\nNote:")
        print("  此脚本在 Skill 中运行时，AI 会自动处理翻译")
    ...
    try:
        ...
        # 创建双语字幕
        # 注意：在实际使用中，AI 会先完成翻译，然后再调用这个函数
        print("\n⚠️  提示：此脚本需要在 Skill 中运行")
        print("   AI 会自动处理翻译逻辑")
        print("   当前仅输出待翻译数据")
